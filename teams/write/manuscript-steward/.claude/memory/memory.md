...
- **glossary-B 정본(정본=`.context/glossary-B-draft.md` v2)**: B판 핵심 개념어 사전의 단일 정본은 `.context/glossary-B-draft.md`. 잠재 구조 ≠ 잠재 상태 ≠ 잠재기제(역행추론 대상) 3축 경계가 논문 전체 축. 에이전트=역행추론 주체(단계 E), 종착=정책 검증(B판 프레이밍). 안정화 후 `register-term`으로 `.team/word.json` 파생 예정. *Derive: term: 잠재기제*
