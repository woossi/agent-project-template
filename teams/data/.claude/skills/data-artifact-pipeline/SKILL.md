...
- 생산된 산출물을 registry(`_data_registry.md`)에 버전·시드·프롬프트·해시와 함께 등록·큐레이션할 때.
...
- 읽는 입력: 작업패킷(`team-inbox`의 `quality_gate.axes`·정지조건), 원천 raw(작업경계 `project/umc` + iCloud `umc-compressed-db` 내), 기존 registry(`_data_registry.md`)·소비 규약(`README_consume.md`).
- 만드는 출력: 압축DB/스키마(data-engineer), 추론 산출물+메타(inference-runner), registry 등록 항목·표/그림(data-curator), cross-team 전달 메시지(`post --to-team write|review`).
...
5. **registry 등록·큐레이션(data-curator).** 산출물을 `_data_registry.md`에 버전·시드·프롬프트·해시로 등록. 외부저장소 의존 제거(원고 내재화 가능 형태). 영향받는 표/그림(ICC·z_shift·ablation) 갱신. 그림은 `figure-designer`/`gis-figure-designer`에 1건씩 위임.
...
- 압축DB·스키마 명세(data-engineer), 추론 산출물+메타(inference-runner), registry 항목·표/그림(data-curator).
