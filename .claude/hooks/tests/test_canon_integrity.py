...
def _claim(rid="C001", evidence=("E001",), status="verified") -> dict:
    return {"claim_id": rid, "claim": "x", "status": status, "evidence": list(evidence)}


def _evidence(rid="E001", provenance=("P001",), status="active") -> dict:
    return {
        "evidence_id": rid,
        "value": "0.49%",
        "label": "l",
...
class ValidateTest(unittest.TestCase):
    ...
    def _full_graph(self) -> None:
        ...
        _write(self.root, "evidence", "E001", _evidence())
    ...
    def test_dangling_link_is_error(self) -> None:
        _write(self.root, "claim", "C001", _claim(evidence=("E999",)))
        result = ci.validate(self.root)
        self.assertTrue(any("dangling" in e for e in result["errors"]))

    def test_deprecated_ref_is_error(self) -> None:
        _write(self.root, "claim", "C001", _claim())
        _write(self.root, "evidence", "E001", _evidence(status="deprecated"))
        _write(self.root, "provenance", "P001", _prov())
        result = ci.validate(self.root)
    ...
    def test_deprecated_ref_rule_is_referrer_gated(self) -> None:
        ...
        # points at a DEPRECATED evidence is a self-consistent retirement, so the
        # deprecated-ref rule (active-referrer only) must NOT fire. The evidence
        ...
        _write(self.root, "evidence", "E001", _evidence(provenance=(), status="deprecated"))
        result = ci.validate(self.root)
    ...
    def test_orphan_evidence_is_warning_not_error(self) -> None:
        # E001 exists but no claim references it; P-less so no dangling.
        _write(self.root, "evidence", "E001", _evidence(provenance=()))
        result = ci.validate(self.root)
        self.assertEqual(result["errors"], [])
    ...
    def test_non_list_link_field_is_error(self) -> None:
        _write(self.root, "claim", "C001", {"claim_id": "C001", "claim": "x",
                                            "status": "draft", "evidence": "E001"})
        result = ci.validate(self.root)
        self.assertTrue(any("must be a list" in e for e in result["errors"]))

    def test_evidence_schema_required_fields_are_errors(self) -> None:
        _write(self.root, "evidence", "E001", {"evidence_id": "E001", "status": "active"})
        result = ci.validate(self.root)
        self.assertTrue(any("evidence schema" in e and "value" in e for e in result["errors"]))
        self.assertTrue(any("evidence schema" in e and "provenance" in e for e in result["errors"]))

    def test_evidence_derived_from_dangling_is_error(self) -> None:
        rec = _evidence(provenance=())
        rec["derived_from"] = ["E999"]
        _write(self.root, "evidence", "E001", rec)
        result = ci.validate(self.root)
        self.assertTrue(any("dangling" in e and "derived_from" in e for e in result["errors"]))

    def test_evidence_derived_from_resolves_to_evidence(self) -> None:
        rec = _evidence(rid="E002", provenance=())
        rec["derived_from"] = ["E001"]
        _write(self.root, "evidence", "E001", _evidence(provenance=()))
        _write(self.root, "evidence", "E002", rec)
        result = ci.validate(self.root)
        self.assertFalse(any("derived_from" in e for e in result["errors"]))

    # ---- supersedes chaining (canon link type 4) ----

    def test_supersedes_dangling_is_error(self) -> None:
        # New evidence supersedes a predecessor that does not exist -> dangling.
        rec = _evidence(rid="E002", provenance=())
        rec["supersedes"] = "E999"
        _write(self.root, "evidence", "E002", rec)
        result = ci.validate(self.root)
    ...
    def test_supersedes_active_predecessor_is_warning_not_error(self) -> None:
        ...
        _write(self.root, "evidence", "E001", _evidence(provenance=(), status="active"))
        rec = _evidence(rid="E002", provenance=())
        rec["supersedes"] = "E001"
        _write(self.root, "evidence", "E002", rec)
        result = ci.validate(self.root)
    ...
    def test_supersedes_retired_predecessor_is_clean(self) -> None:
        ...
        _write(self.root, "evidence", "E001", _evidence(provenance=(), status="replaced"))
        rec = _evidence(rid="E002", provenance=())
        rec["supersedes"] = "E001"
        _write(self.root, "evidence", "E002", rec)
        result = ci.validate(self.root)
    ...
    def test_self_supersedes_is_error(self) -> None:
        rec = _evidence(rid="E001", provenance=())
        rec["supersedes"] = "E001"
        _write(self.root, "evidence", "E001", rec)
        result = ci.validate(self.root)
    ...
    def test_supersedes_null_is_ignored(self) -> None:
        ...
        _write(self.root, "evidence", "E001", _evidence(provenance=()))  # supersedes absent
        result = ci.validate(self.root)
        self.assertFalse(any("supersedes" in e for e in result["errors"]))
...
class FoldTest(unittest.TestCase):
    ...
    def test_fold_empty_kind_is_safe(self) -> None:
        ...
        # one index per canon kind (claim/evidence/provenance + lit_prop/data_registry/runs/risk)
        self.assertEqual(len(written), len(ci.CANON))
        idx = (self.root / ".project/evidence/evidence_index.md").read_text(encoding="utf-8")
        self.assertIn("no records", idx)

    def test_fold_computes_evidence_backref_from_claim(self) -> None:
        # claim C001 cites evidence E001 via evidence; the evidence index must show the
        # reverse link (cited_by=['C001']) WITHOUT E001 storing any back-reference field.
        _write(self.root, "claim", "C001", _claim(evidence=("E001",)))
        _write(self.root, "evidence", "E001", _evidence(provenance=()))
        ci.fold(self.root)
        nidx = (self.root / ".project/evidence/evidence_index.md").read_text(encoding="utf-8")
        self.assertIn("cited_by=['C001']", nidx)
        # And the stored record is untouched (no cited_by field persisted).
        rec = json.loads((self.root / ".project/evidence").glob("E001__*.json").__next__().read_text())
    ...
    def test_backrefs_helper_is_deterministic(self) -> None:
        _write(self.root, "claim", "C002", _claim(rid="C002", evidence=("E001",)))
        _write(self.root, "claim", "C001", _claim(rid="C001", evidence=("E001",)))
        _write(self.root, "evidence", "E001", _evidence(provenance=()))
        back = ci._backrefs(ci._load_all(self.root))
        # Referrers sorted -> C001 before C002 regardless of file discovery order.
        self.assertEqual(back["evidence"]["E001"], ["C001", "C002"])
...
class GuardCliTest(unittest.TestCase):
    ...
    def test_guard_blocks_when_graph_broken(self) -> None:
        _write(self.root, "claim", "C001", _claim(evidence=("E999",)))  # dangling
    ...
    def test_guard_ignores_non_canon_path(self) -> None:
        _write(self.root, "claim", "C001", _claim(evidence=("E999",)))  # broken, but...
    ...
    def test_guard_blocks_non_owner_canon_write(self) -> None:
        _write(self.root, "claim", "C001", _claim(evidence=()))
        r = self._guard(
            str(self.root / ".project/evidence/Enew.json"),
            agent="data-curator",
        )
        self.assertEqual(r.returncode, 2)
        self.assertIn("owner", r.stderr.lower())

    def test_guard_allows_owner_canon_write(self) -> None:
        _write(self.root, "claim", "C001", _claim(evidence=()))
        r = self._guard(
            str(self.root / ".project/evidence/Enew.json"),
            agent="orchestrator",
    ...
    def test_guard_blocks_non_owner_bash_inline_python_canon_write(self) -> None:
        command = (
            "python3 -c \"from pathlib import Path; "
            "Path('.project/evidence/E999__bypass.json').write_text('{}')\""
        )
        ...
        r = subprocess.run(
            ...
            input=payload, capture_output=True, text=True,
            env={
                "CLAUDE_AGENT_NAME": "data-curator",
                "CLAUDE_PROJECT_DIR": str(self.root),
                "PATH": "/usr/bin:/bin",
            },
        )
    ...
    def test_guard_blocks_non_owner_bash_inline_node_canon_write(self) -> None:
        command = (
            "node -e \"require('fs').writeFileSync("
            "'./.project/evidence/E999__bypass.json', '{}')\""
        )
        payload = json.dumps({"tool_name": "Bash", "tool_input": {"command": command},
                              "cwd": str(self.root)})
        r = subprocess.run(
