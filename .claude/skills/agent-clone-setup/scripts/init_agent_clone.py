...
def render_agents_md(contract: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "| `.claude/policies/` | hook이 읽는 기계 판독 정책(JSON) |",
...
def render_claude_md(contract: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "## 컴포넌트 관리",
            "",
            "- `.claude/tasks/tasks.md` — 에이전트가 자동 기록·갱신하는 현재 작업(사용자 큐레이션 금지). 진행 로그와 handoff는 `.context/`에 둔다.",
            "- `.claude/skills/` — 반복되는 작업 묶음을 하나의 포괄 이름으로 승격할 때만 스킬을 만든다. 절차만 담고 작업 로그는 담지 않는다.",
            "- `.claude/agents/` — 특정 스킬 패키지를 독립 컨텍스트에서 관리해야 할 때만 서브에이전트를 만든다. 스킬을 참조하고 절차는 복사하지 않는다.",
            "- `.claude/memory/` — 확정된 장기 맥락만 짧게 남긴다.",
            "- `.claude/policies/` — hook이 읽는 정책(JSON). 결정적으로 유지하고 비밀과 작업 진행은 넣지 않는다.",
