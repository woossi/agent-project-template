...
| **팀 스킬** | 자주 발생하는 **워커 간 워크플로우를 고정** | **별도 시스템.** 트리거 = *워커 간 작업 전달 구조*(누가 누구에게 산출물을 넘기는 반복). 같은 하위 팀 내 ≥2 워커 협업 | `.project/teams/<팀>/skills/` (제안) |
| **프로젝트 스킬** | **팀 간 자료 전달** 구조를 고정 | **별도 시스템.** 트리거 = *팀 간 전달* 반복. 서로 다른 팀 ≥2 | `.project/skills/` (회사 레벨, 제안) |
...
| **팀 메모리** | 하위 팀 합의·맥락 | `.project/teams/<팀>/memory/` (신규, 제안) |
...
- 프로젝트 스킬 디렉토리(`.project/skills`) 실제 도입은 첫 팀 간 전달 스킬이 승격될 때.
