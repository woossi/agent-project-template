...
- **대응 canon 슬롯**: `.project/{claims,evidence,provenance,lit_props}` 4레코드 중 실제(또는 목표) 슬롯. `↔`/`→`는 그래프 엣지.
...
- **그래프 분류** (노드/엣지 측면 — Canon 그래프에서의 역할): 원자를 4값으로 분류한다.
  - **NODE** = 노드 식별자. 레코드를 그래프 노드로 만드는 키(`*_id`). guard가 ID 충돌·prefix를 결정적 검사. (4개: claim/evidence/figure/lit_prop)
  - **EDGE** = 외래참조 간선. 한 노드가 다른 노드(또는 외부 SSOT)를 가리키는 링크. guard가 dangling·deprecated 참조 검사. ADR 강제 간선 3종(claim→evidence, evidence→provenance, provenance→claim) + 설계 간선(→lit_prop, →refs.bib).
  ...
  - **PROP-j+EDGE** = 값(judgment 속성)이면서 동시에 다른 노드로의 간선을 동반(예: result_value는 수치 판단이자 `provenance.value→evidence.value` 간선).

약어 — DE=data-engineer · DC=data-curator · IR=inference-runner · CA=causal-analyst · PS=paper-scout · MW=manuscript-writer · MS=manuscript-steward · SV=stats-validator · QR=quality-reviewer · guard=canon_integrity hook. 팀장: data-lead·analysis-lead·write-lead·scout-lead·review-lead. 묶음: C=claim, Ev=evidence, TF=표·그림, TX=텍스트, L=선행연구.
...
> - `evidence`: evidence_id·value·label·provenance[]·status·supersedes·checked_by·by·ts_ns·note.
...
> - guard `canon_integrity.py` + Rust `canon-enforce`: 7 kind·grounds/counter_grounds/risks 링크·evidence.derived_from 링크·relations cycle(DAG)·bibkey 외부SSOT·source_data/run_id scalar·provenance.derived_from 검사제외·clarity R8/R9/R10 어휘감사. **canon check 0 error/0 warning, hook 회귀 테스트 195 passed.**
...
| 9 | 자료 출처 〔+ Ev.source_name + Ev.산출 자료〕 | C+Ev | DC(레지스트리) | SV | data-lead | provenance.source_data(→D 레지스트리) | ✅구현(D001/D002/D003 실연결) | PROP-j |
| 10 | 자료 시점 〔+ Ev.source_year + Ev.target_period + Ev.산출 연도〕 | C+Ev | DC | SV | data-lead | provenance.source_data.period(신설) | 🔲S6 | PROP-j |
...
| 13 | 측정 척도 〔+ Ev.measurement_scale + Ev.산출 단위 + Ev.단위〕 | C+Ev | DC | SV | data-lead | evidence.label(단위 포함) / provenance.script_or_process | ✅구현(부분) | PROP-j |
| 14 | 지표 산식 〔+ Ev.산식〕 | C+Ev | IR(분석) | SV | analysis-lead | provenance.script_or_process | ✅구현 | PROP-j |
| 15 | 비교 대상 〔+ Ev.comparison_base + Ev.비교 대상〕 | C+Ev | CA(설계) | SV | analysis-lead | claim.components.comparison.baseline | 🔲S3 | PROP-j |
...
| 23 | 결과 크기 〔+ Ev.result_size + Ev.값〕 | C+Ev | IR(생산) | SV(독립 재현) | analysis-lead | evidence.value | ✅구현 | PROP-j |
...
| 27 | 불확실성 〔+ Ev.uncertainty〕 | C+Ev | SV(독립 재현) | SV | review-lead | evidence(신설 uncertainty 필드) / E001b 패턴 | 🔲S6 (현 별도 Ev레코드) | PROP-j |
...
| 35 | 심사자 위험 | C | MW·QR→**QR 정본** | QR | write-lead | risks 레지스트리 ↔ claim.risks / provenance.risks | ✅구현 | PROP-j |
| 36 | 판단동사 | C | MW | QR | write-lead | claim.components.finding.closing_verb(clarity R11) | ✅구현(clarity 감사) | PROP-j |
| 37 | evidence_id | Ev | 생산자(DC/IR) 기입 | guard | 없음 | evidence.evidence_id(=claim.evidence 원소) | ✅구현 | EDGE |
...
| 48 | result_value | Ev | IR(생산) | SV(독립 재현) | review-lead | provenance.value → evidence.value | ✅구현 | PROP-j+EDGE |
| 49 | claim_support_level | Ev | MW(연결 판단) | QR | write-lead | claim.status(draft/supported/verified) | ✅구현 | PROP-j |
| 50 | risk | Ev | QR | QR | review-lead | risks 레지스트리 | ✅구현 | PROP-j |
| 51 | citation | Ev | PS(문헌)·MS(본문)→**MS 정본** | MS·guard | write-lead | lit_prop.bibkey / claim.used_in | 🔲S3·S4 | EDGE |
| 52 | evidence_id | Ev | IR(기입) | guard | 없음 | evidence.evidence_id | ✅구현 | NODE |
| 53 | 수치명 | Ev | MS(정본 보관) | SV | review-lead | evidence.label | ✅구현 | PROP-j |
| 54 | 정본 여부 | Ev | MS(정본 보관) | SV(독립 재현) | review-lead | evidence.status(active/replaced) | ✅구현 | PROP-j |
| 55 | 표/그림 연결 | Ev | DC(생성·파일) | MS·guard | data-lead | provenance.artifact_type=figure/table | ✅구현(슬롯) | EDGE |
| 56 | 원고 위치 | Ev | MS(정본 위치) | MS·guard | write-lead | provenance.manuscript_location | ✅구현 | PROP-s |
| 57 | 검증자 | Ev | SV(검증 주체) | review-lead(원장) | review-lead | evidence.checked_by | ✅구현 | PROP-s |
...
| 73 | 오분류 위험 | TX | SV(오분류 평가) | SV·QR | review-lead | provenance.risks / risks 레지스트리 | ✅구현 | PROP-j |
...
| 9 | claim.자료 출처 + Ev.source_name + Ev.산출 자료 | 3 |
| 10 | claim.자료 시점 + Ev.source_year + Ev.target_period + Ev.산출 연도 | 4 |
| 13 | claim.측정 척도 + Ev.measurement_scale + Ev.산출 단위 + Ev.단위 | 4 |
| 14 | claim.지표 산식 + Ev.산식 | 2 |
| 15 | claim.비교 대상 + Ev.comparison_base + Ev.비교 대상 | 3 |
| 18 | claim.분석 방법 + Ev.method_used | 2 |
| 19 | claim.통제 조건 + Ev.control_condition | 2 |
| 22 | claim.결과 방향 + Ev.result_direction | 2 |
| 23 | claim.결과 크기 + Ev.result_size + Ev.값 | 3 |
| 27 | claim.불확실성 + Ev.uncertainty | 2 |
| 28 | claim.해석 가능 범위 + Ev.interpretive_limit | 2 |
| 75 | 선행연구.저자 + Ev.source_author | 2 |

병합 그룹 13 · 참여 원자 33 · 사라진 행 20. 비병합 의도 구분: `Ev.result_value`(행48, 실제 수치)는 Ev.값/결과크기(행23, 효과 크기)와 의미가 달라 단독 — `provenance.value → evidence.value` 엣지로 연결. `Ev.source_year`는 행10에만 흡수, 선행연구.연도(행76)는 문헌 고유 원자로 단독.
...
| `evidence` | evidence_id (52) | `.project/evidence` (prefix E) | ✅구현 | ID 충돌·prefix |
...
> 주의: figure/table은 ADR에서 독립 노드가 아니라 `provenance.artifact_type`로 표현된다(보조 노드). evidence_id(행37)는 독립 노드키가 아니라 **claim.evidence[] 간선이 가리키는 evidence의 키**이므로 NODE가 아니라 EDGE로 재분류했다.

### B'-2. 간선 (7 EDGE + 2 PROP-j+EDGE) — guard 검사 대상

| 간선 원자(행#) | 출발 노드 | 도착 노드 | canon 링크 | 강제 여부 |
|---|---|---|---|---|
| evidence_id (37) | claim | evidence | `claim.evidence[] → evidence` | ✅guard 강제(ADR 유형1) |
| linked_claim_id (38) | provenance | claim | `provenance.related_claims[] → claim` | ✅guard 강제 |
| result_value (48, +PROP-j) | provenance | evidence | `provenance.value → evidence.value` | ✅guard 강제(evidence.provenance) |
| 표/그림 연결 (55) | evidence | provenance/figure | `evidence → provenance(figure)` | ✅guard 강제(부분) |
...
- 현재 guard가 실제 강제하는 간선은 **4개**(claim→evidence, provenance→claim, provenance→evidence, evidence→provenance 일부) — ADR F-D1 "강제는 3개 링크"와 정합(result_value는 evidence.provenance 경로로 흡수).
...
| 행# | 원자 | 구 분류 | 신 분류 | 사유 |
|---|---|---|---|---|
| 37 | evidence_id | 구조 | EDGE | 독립 노드키 아님 — claim→evidence 간선의 끝점 |
| 38 | linked_claim_id | 구조 | EDGE | provenance→claim 간선 |
| 42 | source_location | 구조 | PROP-s | 위치 속성(간선 아님) |
| 48 | result_value | judgment | PROP-j+EDGE | 값+provenance→evidence 간선 동반 |
| 51 | citation | 구조 | EDGE | →lit_prop 간선 |
| 55 | 표/그림 연결 | 구조 | EDGE | evidence→figure 간선 |
...
| 행48 result_value ↔ 행23 결과 크기/Ev.값 | 둘 다 수치 | **이미 분리됨**(부록 A): result_value=provenance.value(실제값), 결과크기=evidence.value(효과크기). `provenance.value → evidence.value` 간선. |
| 행9·10·13·15(병합행) ↔ evidence/provenance 동의 원자 | claim·Ev 3중 동의 | **부록 A 병합으로 1행화**(이미 처리). canon에선 provenance/evidence 슬롯 1개가 정본, claim은 fold 참조. |
...
|---|---|---|
| ✅구현 — 기반(S0~2) | 44 | claims/evidence/provenance 기반 슬롯 |
...
**실행 결과(ADR §D 슬라이스 로드맵 완수)**: S3(claim 컴포넌트화)·S4(lit_prop)·S5(derived_from+canon_promote)·S6(data_registry/runs)·S7(clarity guard 어휘감사)·잔존(risks) **전부 구현·검증 완료**. hook 회귀 테스트 195 passed. claim 컴포넌트화는 adversarial verify에서 6건 결함(grounds 오정당화·창작 강도/임계값/모집단)을 잡아 재저작·재검증으로 0건까지 수렴.
...
| 35,50,73 | 심사자 위험·risk·오분류 위험 | MW·QR·SV 혼재 | **QR 단독 정본**(위험 판정), 생산자는 입력 제공 | risks 레지스트리 owner를 QR로 두고 `.project/risks/` 레코드로 구현. |
...
| 53,54 | 수치명·정본 여부 | MS·IR·SV 혼재 | **MS 단독 생산**(정본 보관), SV 검증 | ADR: 정본 보관=MS, 독립 검증=SV(E001 레코드 by=MS·checked_by=SV 실측 일치). |

**잔존 미결정(부록 C로 닫지 못함 — owner/사용자 결정 필요, ADR §F와 연동):**
1. **행35·50·73 risks 레지스트리**: `.project/risks/` 레코드와 claim/provenance risks 링크로 구현 완료. 잔여 작업은 새 risk 추가 시 QR owner 판정과 canon gate 통과 여부 확인이다.
...
총 원자 수 **112** = 표 데이터 행 **92** + 병합 흡수 **20**. 그래프 분류: NODE 4 · EDGE 7 · PROP-s 4 · PROP-j+EDGE 2 · PROP-j 75 = 92. **✅구현 92행 · 거버넌스 미생산 0 (S3~S7+risks 전 슬라이스 완수, 2026-06-28).** 생산 역할 미정 0. 이중흡수 0. canon check 0 error/0 warning · hook 회귀 195 passed.
