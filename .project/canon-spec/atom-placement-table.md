...
> 실제 구현 스키마 (대조 기준, 2026-06-28 `.project` 실측 — **S3~S7 구현, risks 클래스 제거 후 갱신**):
...
> - `provenance`: artifact_id·artifact_type·value·source_data(→D)·script_or_process·run_id(→RUN)·related_claims[]·manuscript_location·checked_by·status·manifest_ref·derived_from(off-graph, S5)·by·ts_ns·note.
...
> - guard `canon_integrity.py`: 6 kind·grounds/counter_grounds 링크·relations cycle(DAG)·bibkey 외부SSOT·source_data/run_id scalar·derived_from 검사제외·clarity R8/R9/R10 어휘감사. **canon check 0 error, 회귀 테스트 39 passed.** `risks` 클래스는 제거했다(2026-06-28) — 심사/해석 위험은 claim 본문·해석 영역에서 다룬다.
...
| 35 | 심사자 위험 | C | MW·QR→**QR 정본** | QR | write-lead | claim 본문·해석 영역 / claim.note(risks 클래스 제거) | ✅claim 본문 처리 | PROP-j |
...
| 50 | risk | Ev | QR | QR | review-lead | claim 본문·해석 영역(risks 클래스 제거) | ✅claim 본문 처리 | PROP-j |
...
| 73 | 오분류 위험 | TX | SV(오분류 평가) | SV·QR | review-lead | provenance.note(risks 클래스 제거) | ✅provenance.note 처리 | PROP-j |
...
**결론**: 배치표 구성요소와 guard 스키마 사이의 겹침은 (1)스키마 어긋남 6건 → COMP_SCHEMA 보강으로 해소, (2)표기 오류 1건 → 정정, (3)의미 중복 4건 → 단일 정본 + fold 파생으로 해소. 잔여 충돌 0. 노드 클래스 6종 전부 정합(risks 클래스는 2026-06-28 제거 — 행35·50·73의 위험 원자는 claim 본문·해석 영역·provenance.note로 처리).
...
| ✅구현 — 위험 원자(claim 본문 처리) | 3 | 행35·50·73, claim 본문·해석 영역·provenance.note(risks 클래스 제거) |

(합 = 44+12+20+13+3 = 92 데이터 행, **전부 ✅구현**. 거버넌스 미생산 = **0**. canon check 0 error.)

**실행 결과(ADR §D 슬라이스 로드맵 완수)**: S3(claim 컴포넌트화)·S4(lit_prop)·S5(derived_from+canon_promote)·S6(data_registry/runs)·S7(clarity guard 어휘감사) **전부 구현·검증 완료**. guard 회귀 테스트 39 passed. claim 컴포넌트화는 adversarial verify에서 6건 결함(grounds 오정당화·창작 강도/임계값/모집단)을 잡아 재저작·재검증으로 0건까지 수렴. `risks` 클래스는 2026-06-28 제거 — 위험은 별도 canon 레코드가 아니라 claim 본문·해석 영역에서 다룬다.
...
| 35,50,73 | 심사자 위험·risk·오분류 위험 | MW·QR·SV 혼재 | **QR 단독 정본**(위험 판정), 생산자는 입력 제공 | 위험은 별도 canon 레코드가 아니라 claim 본문·해석 영역·provenance.note에서 다룬다(risks 클래스 2026-06-28 제거). |
...
1. **행35·50·73 risks 레지스트리**: 신설하지 않기로 결정·종결(2026-06-28). `.project/risks/`·`canon_integrity.py`의 risk kind를 제거하고, 위험은 claim 본문·해석 영역·provenance.note에서 다룬다.
...
총 원자 수 **112** = 표 데이터 행 **92** + 병합 흡수 **20**. 그래프 분류: NODE 4 · EDGE 7 · PROP-s 4 · PROP-j+EDGE 2 · PROP-j 75 = 92. **✅구현 92행 · 거버넌스 미생산 0 (S3~S7 전 슬라이스 완수, risks 클래스 제거, 2026-06-28).** 생산 역할 미정 0. 이중흡수 0. canon check 0 error · guard 회귀 39 passed.
