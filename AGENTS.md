...
| `.claude/policies/` | Hook-readable project policy such as agent workspace boundaries (`agent-workspace.json`), promotion conditions (`promotion.json`), and memory derivation conditions (`derivation.json`) | Valid JSON policy files and short operating notes | Keep policy data deterministic and machine-readable. Do not store secrets, personal preferences, task progress, or domain content here. |
